...
HOLE_H = P_Z_C1*2;
	difference() {
		...
		union() {
			...
			translate([-HOLE_X, -HOLE_X, -P_Z_C1]) cylinder(h=HOLE_H, r=HOLE_R, center=false, $fn=100);
// PCB oyuklari
			translate([7.8,  0,  P_Z_ETCH+0.01]) cube([P_ETCH_CONN, P_WIRE_X, P_Z_ETCH], center=true);
			translate([-7.1, 0,  P_Z_ETCH+0.01]) cube([P_ETCH_WIRE, P_WIRE_X, P_Z_ETCH], center=true);	
			
			translate([0,  7.1,  -0.3]) cube([P_WIRE_X, P_ETCH_WIRE, P_Z_ETCH], center=true);
			translate([0, -7.1,  -0.3]) cube([P_WIRE_X, P_ETCH_WIRE, P_Z_ETCH], center=true);	
// wire slits         
         		for(i = [1:55]) translate([-6.1, -5.6 + 2*i/10 , P_Z_ETCH+0.01]) cube([P_ETCH_WIRE, 0.1, P_Z_ETCH], center = true);	
// gas port
			//translate([-HGAS_X,    -HGAS_Y, -P_Z_C1])   cylinder(h=HOLE_H, r=HOLE_R2, center=false, $fn=100);	 
...
module katod2()
{
	difference(){
	union(){
		translate([0, 0, 4]) rotate([0, 0, 90]) anot();
	}
	union(){
	  	for(i = [1:28]) 
			translate([-5.8 + 4*i/10, 6.1 , 4.2]) cube([0.1, 1, 0.2], center = true);	
		for(i = [1:28]) 
			translate([-5.8 + 4*i/10, -6.1 , 4.2]) cube([0.1, 1, 0.2], center = true);	
	}
}
...
module katod2_olculerle(){
translate([ 0, 0, -5]) katod2();
...
// top side
    ...
    union() {
        ...
        translate([-HGAS_X, -HGAS_Y, 0])                      dimensions(HGAS_X,     DIM_LINE_WIDTH*2);
    }
...
translate([ P_LENGTH+3, P_WIDTH, -15])  rotate([90, 0, 180]) scale([1,1,5]) anot();
...
//~~~~~~~~~~~~~~~~~~~~~~~~
// 45 degree 3D view
 translate([ P_LENGTH+3, -2, -5])  rotate([45, 45, 180]) anot();
...
//translate([00, 0, -5]) anot();
//translate([20, 0, -5]) anot();
//translate([40, 0, -5]) 
katod2();
//translate([60, 0, -5]) endcap();
/*
