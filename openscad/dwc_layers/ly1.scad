...
HOLE_H = P_Z_C1*2;
	difference() {
		...
		union() {
			...
			translate([-HOLE_X, -HOLE_X, -P_Z_C1]) cylinder(h=HOLE_H, r=HOLE_R, center=false, $fn=100);
// PCB oyuklari
			translate([0,  7.8,  P_Z_ETCH+0.01]) cube([P_WIRE_X, P_ETCH_CONN, P_Z_ETCH], center=true);
			translate([0, -7.1,  P_Z_ETCH+0.01]) cube([P_WIRE_X, P_ETCH_WIRE, P_Z_ETCH], center=true);		 
		}
	}
}


module katod1_olculerle(){
translate([ 0, 0, -5]) katod1();
...
translate([ P_LENGTH+3, P_WIDTH, -15])  rotate([90, 0, 180]) scale([1,1,5]) katod1();
...
//~~~~~~~~~~~~~~~~~~~~~~~~
// 45 degree 3D view
 translate([ P_LENGTH+3, -2, -5])  rotate([45, 45, 180]) katod1();
